...
%Definition <- %Identifier LEFTARROW %Expression
Expression  <- Choice / Sequence / (ZeroOrOne / ZeroOrMore / OneOrMore) / (Lookahead / NotLookahead / Argument) / Primary
%Choice   <- Expression:1 (SLASH Expression:1)*
%Sequence <- Expression:2 Expression:2*
%ZeroOrOne  <- Expression:3 QUESTION
%ZeroOrMore <- Expression:3 STAR
%OneOrMore  <- Expression:3 PLUS
%Lookahead    <- AND Expression:4
%NotLookahead <- NOT Expression:4
%Argument     <- ARG Expression:4
Primary <- (OPEN %Expression CLOSE) / (%Identifier !LEFTARROW) / %Literal / %Class / %DOT
LEFTARROW  <- '<-' Spacing
SLASH     <- '/' Spacing
ARG       <- '%' Spacing
AND       <- '&' Spacing
